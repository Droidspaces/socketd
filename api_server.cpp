...
void debug_log_request_headers(const std::string &request) {
   ...
   */
  if (visible_len == 0 || request[visible_len - 1] != '\n') {
    std::cerr << '\n';
  }
...
bool send_http_response(int fd, int status_code, const char *reason_phrase,
                        const char *content_type, const std::string &body,
                        bool suppress_body, std::string &error) {
  std::string header;
  header.reserve(256);
...
bool send_empty_http_response(int fd, int status_code,
                              const char *reason_phrase, std::string &error) {
  std::string header;
  header.reserve(192);
...
bool send_bad_request(int fd, bool suppress_body, std::string &error) {
  ...
  return send_http_response(fd, 400, "Bad Request", "application/json", body,
                            suppress_body, error);
}
...
bool send_header_too_large(int fd, bool suppress_body, std::string &error) {
  ...
  return send_http_response(fd, 431, "Request Header Fields Too Large",
                            "application/json", body, suppress_body, error);
}

bool send_not_found(int fd, bool suppress_body, std::string &error) {
  const std::string body = "{\"message\":\"not found\"}\n";
  return send_http_response(fd, 404, "Not Found", "application/json", body,
                            suppress_body, error);
}

bool send_internal_server_error(int fd, const std::string &message,
                                bool suppress_body, std::string &error) {
  std::string body = "{\"message\":\"";
  body += json_escape(message.empty() ? "internal server error" : message);
  body += "\"}\n";

  return send_http_response(fd, 500, "Internal Server Error",
                            "application/json", body, suppress_body, error);
}
...
bool send_ping_ok(int fd, bool suppress_body, std::string &error) {
  ...
  return send_http_response(fd, 200, "OK", "text/plain; charset=utf-8", body,
                            suppress_body, error);
}
...
bool percent_decode_url_path(const std::string &input, std::string &output) {
  output.clear();
...
bool build_static_asset_path(const std::string &target, std::string &file_path,
                             std::string &error) {
  file_path.clear();
  ...
  while (pos <= decoded_path.size()) {
    ...
    const std::size_t end =
        slash == std::string::npos ? decoded_path.size() : slash;
...
bool read_regular_file(const std::string &file_path, std::string &body,
                       bool &not_found, std::string &error) {
  not_found = false;
  body.clear();
...
bool send_static_asset_ok(int fd, const std::string &target, bool suppress_body,
                          std::string &error) {
  ...
  return send_http_response(fd, 200, "OK", content_type.c_str(), body,
                            suppress_body, error);
}

bool is_ascii_digit(char c) { return c >= '0' && c <= '9'; }

bool is_versioned_api_path(const std::string &path, const char *endpoint_path) {
  ...
  if (path.compare(path.size() - endpoint_len, endpoint_len, endpoint_path) !=
      0) {
    return false;
  }

  const std::string prefix = path.substr(0, path.size() - endpoint_len);
...
bool is_api_target(const std::string &target, const char *endpoint_path) {
  const std::size_t query_pos = target.find('?');
  const std::string path =
      query_pos == std::string::npos ? target : target.substr(0, query_pos);

  return path == endpoint_path || is_versioned_api_path(path, endpoint_path);
}
...
bool parse_container_ref_with_suffix(const std::string &target,
                                     const char *suffix, std::string &ref_out) {
  const std::size_t query_pos = target.find('?');
  const std::string path =
      query_pos == std::string::npos ? target : target.substr(0, query_pos);
  ...
  if (unversioned_path.size() <= prefix_len + suffix_len ||
      ...
      unversioned_path.compare(unversioned_path.size() - suffix_len, suffix_len,
                               suffix) != 0) {
    return false;
  }
...
ContainerListRequest parse_container_list_request(const std::string &target) {
  ...
  while (pos <= target.size()) {
    ...
    const std::size_t end = amp == std::string::npos ? target.size() : amp;
...
bool parse_port(const std::string &value, std::uint16_t &port_out,
                std::string &error) {
  if (value.empty()) {
...
bool send_version_ok(int fd, bool suppress_body, std::string &error) {
  ...
  return send_http_response(fd, 200, "OK", "application/json", body,
                            suppress_body, error);
}
...
bool send_container_list_ok(int fd, const std::string &target,
                            bool suppress_body, std::string &error) {
  const ContainerListRequest request = parse_container_list_request(target);
...
bool send_container_inspect_ok(int fd, const std::string &ref,
                               bool suppress_body, std::string &error) {
  std::string body;
  bool not_found = false;
...
bool send_container_start_ok(int fd, const std::string &ref,
                             std::string &error) {
  BackendClient backend;
...
bool send_container_stop_ok(int fd, const std::string &target,
...
bool send_container_restart_ok(int fd, const std::string &target,
                               const std::string &ref, std::string &error) {
  ContainerLifecycleRequest request;
  std::string parse_error;
  if (!parse_container_lifecycle_request(target, request, parse_error)) {
...
bool send_image_list_ok(int fd, bool suppress_body, std::string &error) {
  std::string body;
...
bool send_volume_list_ok(int fd, bool suppress_body, std::string &error) {
  std::string body;
...
bool send_network_list_ok(int fd, bool suppress_body, std::string &error) {
  std::string body;
...
bool send_events_ok(int fd, const std::string &target, bool suppress_body,
                    std::string &error) {
...
bool ApiServer::handle_client(int client_fd, std::string &error) const {
  ...
  if (is_post &&
      parse_container_action_ref(target, "/restart", lifecycle_ref)) {
...
bool ApiServer::run(std::string &error) {
  ...
  // To tty
  std::cerr << "socketd: listening on http://" << config_.bind_address << ':'
            << config_.port << '\n';
  // To API
  ...
  record_socketd_event("daemon", "start", "droidspaces-socketd", attrs,
